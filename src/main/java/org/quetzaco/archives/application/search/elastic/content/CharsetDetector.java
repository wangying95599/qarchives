...
public class CharsetDetector {
	...
	private static CharsetDetector _Instance = null;
	...
	/*
	 * public static void main(String argc[]) { SinoDetect sinodetector; int result
	 * = OTHER; argc = new String[1]; //argc[0] =
	 * "c:\\chinesedata\\codeconvert\\voaunit.txt"; argc[0] = "中文"; sinodetector =
	 * new SinoDetect(); if (argc[0].startsWith("http://") == true) { try { result =
	 * sinodetector.detectEncoding(new URL(argc[0])); } catch (Exception e) {
	 * System.err.println("Bad URL " + e.toString()); } } else { //result =
	 * sinodetector.detectEncoding(new File(argc[0])); result =
	 * sinodetector.detectEncoding(argc[0].getBytes()); }
	 * ConsoleLogger.println(nicename[result]); }
	 */
	...
	public String detectEncoding(File testfile) {
		...
		return nicename[detectEncoding(rawtext)];
	}
	...
	public String detectEncoding(String str) {
		...
		byte[] rawtext;
		try {
			rawtext = str.getBytes("iso8859-1");
			enc = nicename[detectEncoding(rawtext)];
		} catch (Exception e) {
			enc = nicename[0];
			e.printStackTrace();
		}
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			...
			if (rawtext[i] >= 0) {
				...
				if ((byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xF7 && // Original GB range
					...
					row = rawtext[i] + 256 - 0xA1;
					column = rawtext[i + 1] + 256 - 0xA1;
					// ConsoleLogger.println("original row " + row + " column " + column);
					if (GBFreq[row][column] != 0) {
						gbfreq += GBFreq[row][column];
					} else if (15 <= row && row < 55) {
						gbfreq += 200;
					...
					// ConsoleLogger.println("extended row " + row + " column " + column + "
					// rawtext[i] " + rawtext[i]);
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			if (rawtext[i] >= 0) { // in ASCII range
				...
				if (i + 3 < rawtextlen && (byte) 0x8E == rawtext[i] && (byte) 0xA1 <= rawtext[i + 1]
					...
					cnschars++;
					// ConsoleLogger.println("plane 2 or above CNS char");
	...
	int iso_2022_cn_probability(byte[] rawtext) {
		...
		// ConsoleLogger.println("isochars dbchars isofreq totalfreq " + isochars + " "
